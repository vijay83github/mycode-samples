...
$.fn.dataTableExt.oApi.fnFindCellRowIndexes = function ( oSettings, sSearch, iColumn )
{
	...
	var colNames = [];
	$.each( oSettings.aoColumns, function (i, o) {
		colNames[i] = o.mDataProp;
	});

	// loop through all the rows and look for the value
...
//			for ( j=0, jLen=aData.length ; j<jLen ; j++ )
			for ( j=0, jLen=colNames.length ; j<jLen ; j++ )
			{
//				if ( aData[j] == sSearch )
				if ( aData[colNames[j]] == sSearch )
				{
					aOut.push( i );
				}
			}
...
//		else if ( aData[iColumn] == sSearch )
		else if ( aData[colNames[iColumn]] == sSearch )
		{
			aOut.push( i );
		}
	}
