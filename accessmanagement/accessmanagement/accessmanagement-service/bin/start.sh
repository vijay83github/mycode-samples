...
configfile="conf/accessmanagement.yml"
overridefile="accessmanagement.properties"
...
PROJECT_NAME=accessmanagement-service
...
if [ "$MAX_HEAP_SIZE" == "" ]; then
  ...
  if [ "$LT_2G" != "1" ]; then
    ...
    RAW_HEAP_SIZE=`echo $system_memory_in_mb"* .80"|bc -l|sed s/\\\\..*//g`
  	...
  	# take 40% of the memory up to 32G
    RAW_HEAP_SIZE=`echo $system_memory_in_mb"* .40"|bc -l|sed s/\\\\..*//g`
  fi
...
if [ "$JVM_OPTS" == "" ]; then
  ...
  JVM_OPTS="$JVM_OPTS -XX:+OptimizeStringConcat -XX:+UseStringCache"
